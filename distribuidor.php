<?php    if(empty($_GET['data'])){$nwmail=""; $us="";}else{ $nwmail=$_GET['data']; $us=$_GET['user']; }  ?>
...
<script type="text/javascript" src="uk_dat/js/jquery-3.1.1.min.js"></script>
...
body,html{width:100%; height:100%; background:url(uk_dat/img/back.jpg); background-size: 100% 100% cover; background-repeat:repeat; background-position: center center;}

/* CONTACT PAGE */
article{width:100%; max-width:1200px; margin:0 auto; padding:25px 0 80px 0;}
...
form.contacto button {font-size:16px; background: #C00; border: 0 none; color: #FFFFFF; padding:12px; cursor:pointer; letter-spacing:0.08em; -webkit-transition: all 0.2s linear;
...
.msg_ok{background:#CC3;}  .result{background: red;}  #sld_t{width:100%; text-align:center; }  .slider{width:100%;}
#lg{width:240px; height:auto; padding:20px 0;}
...
<strong style="font-size:20px;">Antes de continuar recuerde. </strong>Este formulario es únicamente para clientes que adquieren o vendan suministros de oficina y escolares de la marca Unik&reg;. <strong >Nos reservamos el derecho de revisar y verificar la información ingresada acorde a lo solicitado e indicado.</strong>
<br><br> <div class="bye">Aceptar</div>
</div></div>

<header>  <div class="slider sl_cont"> <!--SLIDERBACK--> <div id="sld_t">
<img src="uk_dat/img/logo2_wh.svg" id="lg" border="0" alt=""/> </div>
</div>  </header>

<!-- PRINCIPAL PAGE -->
<article> 
	<section class="mst-cntact"> <div class="mst-cntxt">
	...
	<strong>(*) Son campos obligatorios.</strong><br> Aplican a registro solo clientes  que adquieren o vendan suministros de oficina y artículos escolares de la marca Unik®.<br>
...
<br>
	...
	<span style="font-size:11px;color:#5B5B5B;">Ingresa a tu perfil de Facebook, Twitter o Instragram desde tu navegador preferido, copia el url de la barra de dirección de tu navegador y pegalo en el campo indicado.</span><br>

	<input type="text" class="face" id="tx" name="face" placeholder="Facebook de tu negocio" autocomplete="off" />
	<input type="text" class="insta" id="tx" name="insta" placeholder="Instagram de tu negocio" autocomplete="off" />
	<input type="text" class="twit" id="tx" name="twit" placeholder="Twitter de tu negocio" autocomplete="off" />
...
</fieldset>  </form>  </div>  </section> </article>


<script type="text/javascript">  /**** FUNCTION FORM CONTACT ****/
$(document).ready(function(){     

	$(".bye").click(function(){	$(".lautxt").remove();});
	$(".boton_envio").click(function() {
		...
		if(nombre == "") {	$(".nombre").focus().after("<span class='error'>Requerido</span>");
		    return false;
			...
			$.ajax({  type: "POST",  url: "mail_dist.php",  data: datos,
	    		success: function() {
					$('.ajaxgif').hide();
